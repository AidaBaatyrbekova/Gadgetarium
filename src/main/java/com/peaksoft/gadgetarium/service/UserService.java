...
public class UserService {
    ...
    public UserResponse createUser(UserRequest request) {
        ...
        if (!request.getPassword().equals(request.getConfirmThePassword())) {
            throw new InvalidPasswordException(ExceptionMassage.PASSWORD_MISMATCH);
        }
    ...
    public LoginResponse login(LoginRequest request) {
        try {
            ...
            throw new BadCredentialsException(ExceptionMassage.AUTHORIZATION_ERROR);
        }

        User user = userRepository.findByEmail(request.getEmail())
                .orElseThrow(() -> new UsernameNotFoundException(ExceptionMassage.AUTHORIZATION_ERROR));
    ...
    public ResponseEntity<String> resetPassword(PasswordResetRequest request) {
        User user = userRepository.findByEmail(request.getEmail())
                .orElseThrow(() -> new UsernameNotFoundException(ExceptionMassage.USER_NOT_FOUND_BY_EMAIL));
    ...
    public ResponseEntity<String> resetPasswordToken(PasswordResetTokenRequest request) {
        ...
        if (!user.getResetPasswordToken().equals(request.getToken())) {
            throw new IllegalArgumentException(ExceptionMassage.INVALID_RESET_TOKEN);
        }

        if (!request.getNewPassword().equals(request.getConfirmNewPassword())) {
            throw new IllegalArgumentException(ExceptionMassage.PASSWORDS_DONT_MATCH);
        }

        if (!isPasswordSecure(request.getNewPassword())) {
            throw new IllegalArgumentException(ExceptionMassage.PASSWORD_NOT_SECURE);
        }
    ...
    public ResponseEntity<String> updatePassword(UpdatePasswordRequest request) {
        ...
        if (!passwordEncoder.matches(request.getPassword(), user.getPassword())) {
            throw new InvalidPasswordException(ExceptionMassage.CURRENT_PASSWORD_INCORRECT);
        }

        if (request.getNewPassword() == null || request.getNewPassword().isEmpty()) {
            throw new InvalidPasswordException(ExceptionMassage.ENTER_NEW_PASSWORD);
        }

        if (request.getNewConfirmPassword() == null || request.getNewConfirmPassword().isEmpty()) {
            throw new InvalidPasswordException(ExceptionMassage.CONFIRM_NEW_PASSWORD);
        }

        if (!request.getNewPassword().equals(request.getNewConfirmPassword())) {
            throw new InvalidPasswordException(ExceptionMassage.PASSWORDS_DONT_MATCH);
        }
    ...
    public Map<String, Object> saveWithGoogle(OAuth2AuthenticationToken oAuth2AuthenticationToken) {
        ...
        if (oAuth2AuthenticationToken == null) {
            throw new NotFoundException(ExceptionMassage.TOKEN_MUST_NOT_BE_NULL);
        }
